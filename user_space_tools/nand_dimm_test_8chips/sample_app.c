...
int main(int argc,char **argv)
{
	...
	uint8_t lun, target,chip,ret;
	uint16_t block, page, length,blk_num =0;
	int16_t blk_cnt;
	float time=0,size = 0,bw=0;
	int chip_no=0, chip_cnt=0;
	if(argc != 5 && argc !=4) {
		printf("Usage: ./<exe_name> <1-PAGE_PROG | 2-PAGE_READ> <chip_no> <chip_cnt> <blk_no(optional)>\n");
		return FAILURE;
	}

	opt = atoi(argv[1]);
	chip_no = atoi(argv[2]);
	chip_cnt = atoi(argv[3]);

	if(chip_no > 7 || chip_no < 0) {
		printf("Chip number between 0 and 7\n");
		return FAILURE;
	}

	if((chip_no + chip_cnt) > 8) {
		printf("Chipno + chip_count should bw less than 8\n");
		return FAILURE;
	}
	if(argc == 5){
		blk_cnt = atoi(argv[4]);
			...
			for(block = 0; block < blk_cnt; block++){
				for(lun = 0; lun< NAND_LUN_COUNT; lun++){
					for( target = 0; target < NAND_TARGET_COUNT; target++){
						for (chip = chip_no; chip < (chip_no+chip_cnt) ; chip++) {
					...
					for(lun = 0; lun < NAND_LUN_COUNT; lun++){
						for(target = 0; target < NAND_TARGET_COUNT; target++){
							for(chip = chip_no; chip < (chip_no + chip_cnt); chip++){
								...
								cmd->col_addr = 0x0;
								cmd->page = page;
								cmd->host_addr[0] = (uint64_t) phy_addr[2];
								cmd->host_addr[1] = (uint64_t) (phy_addr[2]+0x1000);
								cmd->host_addr[2] = (uint64_t) (phy_addr[2]+0x2000);
								cmd->host_addr[3] = (uint64_t) (phy_addr[2]+0x3000);
								cmd->host_addr[4] = 0;
								...
								cmd->col_addr = 0x0;
								cmd->page = page;
								cmd->host_addr[0] = (uint64_t) (phy_addr[2]+0x5000);
								cmd->host_addr[1] = (uint64_t) (phy_addr[2]+0x6000);
								cmd->host_addr[2] = (uint64_t) (phy_addr[2]+0x7000);
								cmd->host_addr[3] = (uint64_t) (phy_addr[2]+0x8000);
								cmd->host_addr[4] = 0;
	...
	size = ((float)(2 * 2 * chip_cnt * 16 * NAND_PAGES_PER_BLOCK * blk_cnt))/1024;
