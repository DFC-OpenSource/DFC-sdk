...
extern int cpld_read(unsigned chip_addr, unsigned reg_addr);
static int cpld_version(void)
{
	u8 ret1, ret2;
	if((i2c_probe(CONFIG_SYS_I2C_MUX_ADDR)) == 0) {
		mux_write( 0x77, 0x00, 0x0f);         /*configuring to channel 8 of MUX 1*/
		if((i2c_probe(CONFIG_SYS_I2C_CPLD_ADDR)) == 0) {
			ret1 = cpld_read( CONFIG_SYS_I2C_CPLD_ADDR, 0x01);
			if( ret1 != -1 ) {
				printf("Board Arch: V%d, Board Version: %c\n", ret1, ((ret1 & 0x0f) +'A' -1));
			}
			ret2 = cpld_read( CONFIG_SYS_I2C_CPLD_ADDR, 0x02);
			if( ret2 != -1 ) {
				printf("CPLD_NIC Version: %c%u\n",((ret2 >> 5) + 'A'), ret2 & 0x1f);
			}
		}
		else {
			printf("CPLD_NIC is not probed on MUX1 Channel 8\n");
		}
	}
	else {
		printf("I2C MUX1 is not detected\n");
	}
	mux_write( 0x77, 0x00, 0x08);     /* Reverting the MUX configuration*/
	return 0;
}
#endif
...
__weak ulong board_reserve_ram_top(ulong ram_size)
{
