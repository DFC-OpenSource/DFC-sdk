...
#ifdef CONFIG_ISSD
	if(!strcmp(name, "bootimg")){
		if(!strcmp(value, "issd")){
			char *argv1[]={"setenv", "bootcmd", CONFIG_BOOTCOMMAND_ISSD};
			char *argv2[]={"setenv", "bootargs", CONFIG_BOOTARGS};
			_do_env_set(0, 3, argv1, H_INTERACTIVE);
			_do_env_set(0, 3, argv2, H_INTERACTIVE);
			printf("Please do saveenv and Restart the Host machine\n");
		}else if(!strcmp(value, "issd-sa")){
			char *argv1[]={"setenv", "bootcmd", CONFIG_BOOTCOMMAND_ISSD};
			char *argv2[]={"setenv", "bootargs", CONFIG_BOOTARGS_ISSD_SA};
			_do_env_set(0, 3, argv1, H_INTERACTIVE);
			_do_env_set(0, 3, argv2, H_INTERACTIVE);
			printf("Please do saveenv and Restart the Host machine\n");
		}else if(!strcmp(value, "inic")){
			char *argv1[]={"setenv", "bootcmd", CONFIG_BOOTCOMMAND_INIC};
			char *argv2[]={"setenv", "bootargs", CONFIG_BOOTARGS_INIC};
			_do_env_set(0, 3, argv1, H_INTERACTIVE);
			_do_env_set(0, 3, argv2, H_INTERACTIVE);
			printf("Please do saveenv and Restart the Host machine\n");
		}else{
			printf("Wrong bootimg: %s Option: inic | issd | issd-sa\n", value);
		}
        }
	free(value);
#endif
