...
void ls2080a_handle_phy_interface_sgmii(int dpmac_id)
{
	...
	case 0x4D:
		...
		case 3:
			/* Slot housing a SGMII riser card? */
			if (slot == EMI_NONE) {
				printf("%s %d: EMI NONE dpmac_id=%d\n",
					__func__, __LINE__, dpmac_id);
				return;
			}
			if (serdes1_prtcl == 0x39) {
				wriop_set_phy_address(dpmac_id,
					      riser_phy_addr[dpmac_id - 2]);
				if (dpmac_id >= 6 && hwconfig_f("xqsgmii", env_hwconfig))
					wriop_set_phy_address(dpmac_id,
					      riser_phy_addr[dpmac_id - 3]);
			} else {
				wriop_set_phy_address(dpmac_id,
					      riser_phy_addr[dpmac_id - 2]);
				if (dpmac_id >= 7 && hwconfig_f("xqsgmii", env_hwconfig))
					wriop_set_phy_address(dpmac_id,
					      riser_phy_addr[dpmac_id - 3]);
			}
	...
	case 0x57:
		...
		case 5:
			...
			if (serdes2_prtcl == 0x47) {
				...
				if (dpmac_id >= 14 && hwconfig_f("xqsgmii", env_hwconfig))
					wriop_set_phy_address(dpmac_id,
