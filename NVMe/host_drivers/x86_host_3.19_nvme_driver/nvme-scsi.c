							...
							u8 *cmd)
{
	...
	parm_list_length =
		...
		printk(KERN_DEBUG "Passed %s\nlength : %u\tdwords : %u\n",__func__,parm_list_length,BYTES_TO_DWORDS);
        printk(KERN_DEBUG " length mod dwords :  %d\n",parm_list_length % BYTES_TO_DWORDS);
	...
	case DOWNLOAD_SAVE_ACTIVATE:
		printk(KERN_DEBUG " Download save activate \n");
	...
	case DOWNLOAD_SAVE_DEFER_ACTIVATE:
		printk(KERN_DEBUG " Download save defer activate \n");
		res = nvme_trans_send_fw_cmd(ns, hdr, nvme_admin_download_fw,
						parm_list_length, buffer_offset,
						buffer_id);
		break;
	case ACTIVATE_DEFERRED_MICROCODE:
		printk(KERN_DEBUG " activate deferred microcode \n");
		res = nvme_trans_send_fw_cmd(ns, hdr, nvme_admin_activate_fw,
						parm_list_length, buffer_offset,
						buffer_id);
		break;
...
static int nvme_scsi_translate(struct nvme_ns *ns, struct sg_io_hdr *hdr)
{
	...
	case WRITE_12:
	case WRITE_16:
		printk(KERN_DEBUG "WRITE_16 : %d\n",__LINE__);
		retcode = nvme_trans_io(ns, hdr, 1, cmd);
		break;
	case INQUIRY:
		printk(KERN_DEBUG "INQUIRY : %d\n",__LINE__);
		retcode = nvme_trans_inquiry(ns, hdr, cmd);
		break;
	case LOG_SENSE:
		printk(KERN_DEBUG "LOG_SENSE : %d\n",__LINE__);
		retcode = nvme_trans_log_sense(ns, hdr, cmd);
		break;
	case MODE_SELECT:
	case MODE_SELECT_10:
		printk(KERN_DEBUG "MODE_SELECT : %d\n",__LINE__);
		retcode = nvme_trans_mode_select(ns, hdr, cmd);
		break;
	case MODE_SENSE:
	case MODE_SENSE_10:
		printk(KERN_DEBUG "MODE_SENSE : %d\n",__LINE__);
		retcode = nvme_trans_mode_sense(ns, hdr, cmd);
		break;
	case READ_CAPACITY:
		printk(KERN_DEBUG "READ_CAPACITY : %d\n",__LINE__);
		retcode = nvme_trans_read_capacity(ns, hdr, cmd);
		break;
	case SERVICE_ACTION_IN_16:
		printk(KERN_DEBUG "SERVICE_ACTION_IN : %d\n",__LINE__);
		...
		break;
	case REPORT_LUNS:
		printk(KERN_DEBUG "REPORT_LUNS : %d\n",__LINE__);
		retcode = nvme_trans_report_luns(ns, hdr, cmd);
		break;
	case REQUEST_SENSE:
		printk(KERN_DEBUG "REQUEST_SENSE : %d\n",__LINE__);
		retcode = nvme_trans_request_sense(ns, hdr, cmd);
		break;
	case SECURITY_PROTOCOL_IN:
	case SECURITY_PROTOCOL_OUT:
		printk(KERN_DEBUG "SECURITY_PROTOCOL_OUT : %d\n",__LINE__);
		retcode = nvme_trans_security_protocol(ns, hdr, cmd);
		break;
	case START_STOP:
		printk(KERN_DEBUG "START_STOP : %d\n",__LINE__);
		retcode = nvme_trans_start_stop(ns, hdr, cmd);
		break;
	case SYNCHRONIZE_CACHE:
		printk(KERN_DEBUG "SYNCHRONIZE_CACHE : %d\n",__LINE__);
		retcode = nvme_trans_synchronize_cache(ns, hdr, cmd);
		break;
	case FORMAT_UNIT:
		printk(KERN_DEBUG "FORMAT_UNIT : %d\n",__LINE__);
		retcode = nvme_trans_format_unit(ns, hdr, cmd);
		break;
	case TEST_UNIT_READY:
		printk(KERN_DEBUG "TEST_UNIT_READY : %d\n",__LINE__);
		retcode = nvme_trans_test_unit_ready(ns, hdr, cmd);
		break;
	case WRITE_BUFFER:
		printk(KERN_DEBUG "WRITE_BUFFER  : %d\n",__LINE__);
		retcode = nvme_trans_write_buffer(ns, hdr, cmd);
		break;
	case UNMAP:
		printk(KERN_DEBUG "UNMAP  : %d\n",__LINE__);
		retcode = nvme_trans_unmap(ns, hdr, cmd);
		break;
	default:
 out:
		printk(KERN_DEBUG "default/out  : %d\n",__LINE__);
