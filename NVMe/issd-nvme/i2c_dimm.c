...
int ns_check_valid(NvmeCtrl *n,int slot[])
{
	if(!(n->dm[slot[0]]->valid || n->dm[slot[1]]->valid || n->dm[slot[2]]->valid || n->dm[slot[3]]->valid)) {
		syslog(LOG_ERR,"\nERROR: No dimm modules...\n\n");
		return -1;
	}
	if(n->dm[slot[3]]->valid && n->dm[slot[2]]->valid) {
		if(n->dm[slot[1]]->valid ) {
			if (n->dm[slot[0]]->valid) {
...
int get_dimm_info(NvmeCtrl *n)
{
				...
				if((check & 255) != 255) {  /*TODO Check with DDR/NAND device id*/
					...
					if(n->fpga_version == 0) {
						...
						if (!(device_count & 2)) { 
							...
							printf("ERROR: DDRs should be in M1 & M2.\nPlease shutdown and Put DDRs on M1 & M2  and Put NANDs on M3 & M4 then Power it again.\n");
							return -1;
						}
					}
				} else {
					...
					if((n->fpga_version ==0) && (device_count & 1)) {
						printf("ERROR: DDRs should be in M4 & M2.\nPlease shutdown and Put DDRs on M4 & M2  and Put NANDs on M3 & M1 then Power it again.\n");
						return -1;
					} 
					if ((n->fpga_version == 1) && (!(device_count & 2)))  {
						printf("ERROR: DDRs should be in M1 & M2.\nPlease shutdown and Put DDRs on M1 & M2  and Put NANDs on M3 & M4 then Power it again.\n");
						return -1;
					}
				}
	...
	if (n->fpga_version == 1) {
		...
		ret=ns_check_valid(n,slot);

	if(ret) {
		printf("Check the DIMM modules present or not\n");
		return -1;
	}
	close(i2c1_file);
...
int get_new_dimm_info(NvmeCtrl *n)
{
	dimm_details *dm;
	int dimm_slot[4] = {0x51,0x52,0x53,0x54}; /*I2C ADDR: 52,54-->PEX2 DDR; 51,53-->PEX4 NAND*/
	int check = 0,ret;
...
#if 0
	if((n->fpga_version != 0xa3030500) && (n->fpga_version != 0xa7030500) && (n->fpga_version != 0xa3030501) && (n->fpga_version != 0xa7030501)) {
		int slot[4]={0,1,2,3};
		ret=ns_check_valid(n,slot);
	}
	else {
