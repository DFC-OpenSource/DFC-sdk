...
#endif
#define MODIFIED_SEARCH
#define QDMA_WORK_QUEUE
#ifdef QDMA_WORK_QUEUE
#include <linux/workqueue.h>

struct workqueue_struct *qdma_wq = NULL;
typedef struct
{
    struct work_struct qdma_work;
...
static void dpaa2_qdma_fqdan_cb(struct dpaa2_io_notification_ctx *ctx)
{
...
#ifdef MODIFIED_SEARCH
    static int k,fd_cnt,fd_cnt_tmp,found1;
    struct dpaa2_qdma_comp *dpaa2_comp[n_chans], *_comp_tmp[n_chans],*temp_dpaa2_comp;
    struct dpaa2_qdma_chan *qchan[n_chans];
    const struct dpaa2_fd *fd_addr[2];
    int check[n_chans];
    ...
    for (i = 0; i < n_chans; i++) 
        qchan[i] = &priv->dpaa2_qdma->chans[i];
START:
    fd_cnt = 0;
    while (!is_last) {
        do {
    ...
    for (i = 0; i < n_chans; i++) {
        check[i]=0;
        spin_lock(&qchan[i]->queue_lock);
        if (list_empty(&qchan[i]->comp_used)) {
            check[i] = 1;
            spin_unlock(&qchan[i]->queue_lock);
            continue;
        }
        dpaa2_comp[i] = list_first_entry(&qchan[i]->comp_used,typeof(*dpaa2_comp[i]),list);
        _comp_tmp[i] = list_next_entry(dpaa2_comp[i],list);
        spin_unlock(&qchan[i]->queue_lock);
    }
    while(1) {
        ...
        for (i = 0; i < n_chans; i++) {
            ...
            if(check[i]) 
                continue;
            spin_lock(&qchan[i]->queue_lock);
            fd_eq = (struct dpaa2_fd *)dpaa2_comp[i]->fd_virt_addr;
            switch(fd_cnt_tmp)
            {
                case 1:
                    if (unlikely((fd_eq->simple.addr_lo == fd_addr[k]->simple.addr_lo) && (fd_eq->simple.addr_hi == fd_addr[k]->simple.addr_hi)))  {
                        fd_cnt_tmp--;
                        found1=1;
                        break;
                    }
                    goto GET_NEXT_ENTRY;
                case 2:
                    if (unlikely(((fd_eq->simple.addr_lo == fd_addr[0]->simple.addr_lo) && (fd_eq->simple.addr_hi == fd_addr[0]->simple.addr_hi)) || ((fd_eq->simple.addr_lo == fd_addr[1]->simple.addr_lo) && (fd_eq->simple.addr_hi == fd_addr[1]->simple.addr_hi)))) {
                        k = ((fd_eq->simple.addr_lo == fd_addr[0]->simple.addr_lo) && (fd_eq->simple.addr_hi == fd_addr[0]->simple.addr_hi)) ? 1 : 0;
                        fd_cnt_tmp--;
                        found1=1;
                        break;
                    }
                    goto GET_NEXT_ENTRY;
                default:
                    printk("Something goes wrong %s:%d fd_cnt: %d",__func__,__LINE__,fd_cnt);
                    goto START;
            }
            list_del(&dpaa2_comp[i]->list);
            list_add_tail(&dpaa2_comp[i]->list,
                    &qchan[i]->comp_free);

            spin_lock(&qchan[i]->vchan.lock);
            vchan_cookie_complete_for_qdma(
                    &dpaa2_comp[i]->vdesc);
            spin_unlock(&qchan[i]->vchan.lock);
            found++;
GET_NEXT_ENTRY:
            if(!found1) {
                if(&dpaa2_comp[i]->list != &qchan[i]->comp_used) { /*all the list is done*/
                    dpaa2_comp[i] = _comp_tmp[i];
                    _comp_tmp[i] = list_next_entry(_comp_tmp[i],list);
                }
                else {
                    check[i] = 1; /*mark for not going again into circular linked list*/
                }
            }
            spin_unlock(&qchan[i]->queue_lock);
            if (found == fd_cnt ) 
...
#ifdef QDMA_WORK_QUEUE
void qdma_workqueue_init(void)
{
    qdma_wq = create_singlethread_workqueue("QDMAworkqueue");
    if(qdma_wq) {
        printk("QDMA Work queue is initialized!\n");
    }
    else {
        printk("QDMA Work queue initialization failed!\n");
    }

}

void wq_handler (struct work_struct *WQ) 
{
    workq_t *tmp = (workq_t *) WQ;
    dpaa2_qdma_fqdan_cb(tmp->ctx);        
#if 0
    workq_t *tmp = NULL;
    tmp = container_of(WQ,workq_t,qdma_work);
    dpaa2_qdma_fqdan_cb(tmp->ctx);        
#endif
    kfree((void*)WQ);
    return;
...
void qdma_work_queue_cb(struct dpaa2_io_notification_ctx *ctx) 
{
    workq_t *wq = (workq_t*) kmalloc(sizeof(workq_t),GFP_KERNEL);
    wq->ctx = ctx;
    INIT_WORK((struct work_struct*)wq,wq_handler);
    queue_work(qdma_wq,(struct work_struct *)wq);
}
#endif
...
#ifndef QDMA_WORK_QUEUE
		ppriv->nctx.cb = dpaa2_qdma_fqdan_cb;
#else
        ppriv->nctx.cb = qdma_work_queue_cb;
#endif		
...
{
#ifdef QDMA_WORK_QUEUE    
    printk("Workqueue is initializing\n");
    qdma_workqueue_init();
#endif
...
{
#ifdef QDMA_WORK_QUEUE    
    destroy_workqueue(qdma_wq);
    printk("Workqueue is destroyed");
#endif
